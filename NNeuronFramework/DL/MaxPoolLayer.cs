...
{
    public class MaxPoolLayer : BaseLayer
    {
        private int rows;
        private int cols;

        public MaxPoolLayer(int rows, int cols)
        {
            this.rows = rows;
            this.cols = cols;
        }
        ...
        private Matrix Conv(Matrix input, int rows, int cols)
        {
            ...
            for (var row = 0; row < targetRows; row++)
            {
                for (var col = 0; col < targetCols; col++)
                {
                    ...
                    double value = CalcConv_Max(input, row, col, rows, cols);
        ...
        private double CalcConv_Max(Matrix input, int beginRow, int beginCol, int calc_rows, int calc_cols)
        {
            double maxValue = input.Data[beginRow + 0][beginCol + 0];
